...
int command, previousCommand = 2, currentSpeed = 10;
// ~~~~~~~~~~~~~~~~~~~~~ \\ 
...
{ 
  // put your main code here, to run repeatedly:
   ...
   // get the input from the controller and depending on the input, do stuff
  if (radio.available()) 
  {
  ...
  // command the robot depending on the command variable
  ...
  switch(command)
  {
    case 0: 
    {
      if(previousCommand == 0) break;
      Stretch();
      break;
    }
    case 1: 
    {
      if(previousCommand == 1) break;
      Crouch();
      break;
    }
    case 2:
    {
      if(previousCommand == 2) break;
    ...
    case 5:
    {
      if(previousCommand != 5)
      { // if its already walking forward
         prepareForMovement(1, currentSpeed);  
      }
      // ADJUST AMOUNT OF TIMES!!!!
      moveForward(1, currentSpeed);
      break;
    }
    case 6:
    {
      if(previousCommand != 6)
      { // if its already walking forward
         prepareForMovement(0, currentSpeed); 
      }
      // ADJUST AMOUNT OF TIMES!!!!
      moveForward(0, currentSpeed);
      break;
    }
    case 7:
    {
      if(previousCommand != 7)
      { // if its already going right
        prepareForRotation(1, currentSpeed);
      }
      // ADJUST AMOUNT OF TIMES!!!!
      Rotate(1, currentSpeed);
      break;
    }
    case 8:
    {
      if(previousCommand != 8)
      { // if its already going right
        prepareForRotation(0, currentSpeed);
      }
      // ADJUST AMOUNT OF TIMES!!!!
      Rotate(0, currentSpeed);
      break;
    }    
  }
  // save the previus command
